...
module.exports = async function (req, res, next) {
    ...
    if (empty(result)) {
        ...
        if (!empty(result) && (
            result[0].Description.toLowerCase() == 'super admin' ||
            result[0].Description.toLowerCase() == 'admin' ||
            result[0].Description.toLowerCase() == 'cs admin'
        )) {
            //proceed if super admin, admin, RO user or cs admin
            next()
        } else {
            logger.error("[verifyCSAdmin] :: error : Unauthorized");
            res.status(401).json({
                status: {
                    code: 401,
                    name: "Unauthorized",
